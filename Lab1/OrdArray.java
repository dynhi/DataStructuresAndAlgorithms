...
public class OrdArray {
    ...
    public OrdArray merge(OrdArray inputArray){
        OrdArray mergedArray = new OrdArray(this.size() + inputArray.size());
        int thisIndex = 0;
        int inputIndex = 0 ;
        int targetIndex = 0;
        ...
        while(thisIndex < this.size() && inputIndex < inputArray.size()){
            if(this.a[thisIndex] > inputArray.a[inputIndex]){
                mergedArray.a[targetIndex] = inputArray.a[inputIndex];
                inputIndex++;
            }
            else {
                mergedArray.a[targetIndex] = this.a[thisIndex];
                thisIndex++;
            }
            targetIndex++;
        }
        ...
        mergedArray.nElems = targetIndex;
        return mergedArray;
    }
}
